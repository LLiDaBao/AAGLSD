...
/* @brief The perpendicular distance from the 
mid point of line segment to reference line segment. */
inline
float perpendCenterDist(
	const LineSegment& _seg, 
	const LineSegment& ref
)
{
	Pixel _midPx((_seg.begPx + _seg.endPx) * 0.5);
	return ref.point2lineDist(_midPx);
}
