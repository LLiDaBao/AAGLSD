...
)
{
	...
	case PREWITT:
		kx = (cv::Mat_<float>(3, 3) << -1.f, 0.f, 1.f, -1.f, 0.f, 1.f, -1.f, 0.f, 1.f);
		ky = (cv::Mat_<float>(3, 3) << -1.f, -1.f, -1.f, 0.f, 0.f, 0.f, 1.f, 1.f, 1.f);
...
)
{
	...
	for (int row = 1; row < gradx.rows - 1; ++row)
	{
		for (int col = 1; col < gradx.cols - 1; ++col)
		{
			...
			if (currMag > temp1 && currMag > temp2 && currMag >= MIN_GRAD_THRESH)
			{
